...
Validates an email address meets pragmatic format and length expectations. By design, rejects RFC-valid emails that are unlikely to appear in the wild.
...
### 1\. Consumer Email Services
  ...
  * **Gmail**:
      ...
      * **Length**: Local part must be **6-30 characters**.
...
These services sit in front of millions of corporate logins. If they reject it, the user can't sign up.
  ...
  * **Auth0**:
      ...
      * **Source**: [Auth0 Email Field Limits](https://support.auth0.com/center/s/article/Email-length)
  * **Okta**:
      * **Length**: The standard User Profile email field is limited to **100 characters** in the console.
      * **Restricted Chars**: Explicitly forbids most special characters in the local part except `.` `_` `-` `+`.
      * **Source**: [Okta Character Limits](https://www.google.com/search?q=https://support.okta.com/help/s/article/What-Are-the-Character-Limits-for-Certain-Fields-in-the-Okta-Console)
  * **Microsoft Entra ID (Azure AD)**:
      * **Length**: The `UserPrincipalName` (UPN) is limited to **113 characters** total (64 before the `@` and 48 after).
      * **Blocklist**: Rejects nearly all "special" characters allowed by RFC (like `! # $ % * / ? ^ { } | ~`) in the creation flow.
      * **Source**: [Prepare for Directory Synchronization (Microsoft Learn)](https://learn.microsoft.com/en-us/microsoft-365/enterprise/prepare-for-directory-synchronization?view=o365-worldwide)
...
If your user inputs an email, you likely send it via one of these. If they reject it, your app breaks.
  ...
  * **AWS SES**:
      * **Length**: Max **320 characters** total (64 local, 255 domain).
      ...
      * **Source**: [AWS SES FAQ](https://aws.amazon.com/ses/faqs/)
  * **SendGrid**:
...
Some of the world's most popular toolkits are also pragmatic.
